...
export default function Step1Country({ updateForm }) {
  const { countryCode, config, switchCountry } = useCountry()
  ...
  return (
    <Shell>
      {/* ── Header — clean, no horizontal stepper ─────────────── */}
      <div className="mb-8 pb-6 border-b border-[#1e293b]">
        ...
        <p className="text-sm text-[#94a3b8]">Choose the target region to calibrate the economic mapping model.</p>
      </div>

      {/* ── Cards — 3 horizontal, redesigned checkboxes & labels ── */}
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5 mb-8">
        {Object.values(COUNTRY_CONFIGS).map((cfg) => {
          const active = countryCode === cfg.code
          return (
            <button
              key={cfg.code}
            ...
            >
              {/* Top row: country code + name, checkbox top-right */}
              <div className="flex justify-between items-start mb-5">
                ...
                {/* Checkbox — filled mint on active, visible light-gray border on inactive */}
              ...
              {/* Bottom row: CURRENCY + CALIBRATION — enlarged, legible light-gray */}
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between bg-[#1a2332] border border-[#2a3444] rounded-lg px-6 py-4">
        ...
        {/* Right: CTA button — flush right */}
        <button
          onClick={handleNext}
          className="mt-4 sm:mt-0 px-7 py-2.5 bg-[#1aA882] hover:bg-[#12967A] text-white text-[13px] font-semibold rounded-md transition-colors flex items-center gap-2 shrink-0"
        >
          Save & Continue
